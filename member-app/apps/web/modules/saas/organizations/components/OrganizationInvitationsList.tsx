...
export function OrganizationInvitationsList({
	...
	const invitations = useMemo(
		() =>
			organization?.invitations
				...
				.sort(
					(a: { expiresAt: Date }, b: { expiresAt: Date }) =>
	...
	const revokeInvitation = (_invitationId: string) => {
		toast.promise(
			async () => {
				// Organization plugin removed - cancelInvitation functionality disabled
				// TODO: Implement via ORPC if needed
				throw new Error(
					"Organization invitation cancellation is not available. Organization plugin has been removed.",
				);
			},
			{
				loading: t(
	...
	>[] = [
		{
			...
			cell: ({ row }) => {
				const InvitationStatusIcon = ({
					...
					rejected: XIcon,
					canceled: XIcon,
				}[row.original.status] || ClockIcon) as typeof ClockIcon;
				return (
