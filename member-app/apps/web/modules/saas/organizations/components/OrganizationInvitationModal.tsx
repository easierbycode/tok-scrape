...
import { CheckIcon, XIcon } from "@/modules/ui/icons";
...
export function OrganizationInvitationModal({
	...
	const onSelectAnswer = async (accept: boolean) => {
		...
		try {
			if (accept) {
				// Organization plugin removed - acceptInvitation functionality disabled
				// TODO: Implement via ORPC if needed
				throw new Error(
					"Organization invitation acceptance is not available. Organization plugin has been removed.",
				);
			}
			// Organization plugin removed - rejectInvitation functionality disabled
			// TODO: Implement via ORPC if needed
			throw new Error(
				"Organization invitation rejection is not available. Organization plugin has been removed.",
			);
		} catch {
			// TODO: handle error
		} finally {
