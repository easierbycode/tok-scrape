// Order-detail template manager — Samples-Import fork.
//
// This is a fork of mobile-demo/www/ext/ext/template.js for the Samples-Import
// demo, with ONE change: show() no longer dips the frame opacity to 0.35 between
// products (the original "pop" flickered the modal contents product-to-product
// during a fast import loop). It keeps only the subtle scale pop. Everything
// else — the iframe render of fixtures/order.html, the field fills — is identical
// to the original so the two apps share the same visual component.
//
...
(function () {
  ...
  function show(order) {
    ...
    // Subtle scale pop only — NO opacity dip (the original dipped to 0.35, which
    // flickered the modal as the import loop swapped products). Opacity stays 1.
    frame.style.transition = 'none';
    frame.style.opacity = '1'; frame.style.transform = 'scale(0.985)';
    requestAnimationFrame(() => {
      frame.style.transition = 'transform 0.35s cubic-bezier(.2,.8,.2,1)';
